...
channelDict={
    "2016":{"mt":MTSM2016(),"et":ETSM2016(), "tt":TTSM2016(), "em":EMSM2016()},
    "2017":{"mt":MTSM2017(),"et":ETSM2017(), "tt":TTSM2017(), "em":EMSM2017()},
    "2018":{"mt":MTSM2018(),"et":ETSM2018(), "tt":TTSM2018(), "em":EMSM2018()}
}
eraD={
...
def cutDB(meta,cutName):
    if cutName=="tauDisc":
        if meta["channel"] in ["et","mt"]:
            ...
            c=Cut("(byMediumDeepTau2017v2p1VSjet_2>0.5&&byMediumDeepTau2017v2p1VSjet_1<0.5)||(byMediumDeepTau2017v2p1VSjet_1>0.5&&byMediumDeepTau2017v2p1VSjet_2<0.5)", "tauDisc")
...
trainingVarL={"bpt_1", "dijetpt", "eta_1", "jdeta", "jpt_1", "jpt_2", "ME_q2v1", "ME_q2v2", "mjj", "m_sv", "m_sv_puppi", "mTdileptonMET", "mTdileptonMET_puppi", "m_vis", "nbtag", "njets", "pt_1", "pt_2", "ptvis"}
...
class jetFakeEstimation(object):
    def __init__(self,eraName,channelName, bkgName):
        ...
        if self.meta["channel"] in ["et","mt"]:
            ...
            ##switch to deeptauID
            self.channel.cuts.get("againstMuonDiscriminator").variable = "byTightDeepTau2017v2p1VSmu_2"
            self.channel.cuts.get("againstElectronDiscriminator").variable = "byVLooseDeepTau2017v2p1VSe_2"
            self.channel.cuts.remove("trg_selection")
            self.channel.cuts.add(Cut("(pt_2>30 && ((trg_singlemuon == 1) || (trg_mutaucross == 1)))","trg_selection"))
            #self.channel.cuts.add(Cut("byVLooseIsolationMVArun2017v2DBoldDMwLT2017_2>.5","VLooseTauIso"))
            self.channel.cuts.add(Cut("byVLooseDeepTau2017v2p1VSjet_2>.5","VLooseTauIso"))
        ...
        if self.meta["channel"] =="tt":
            ...
            self.channel.cuts.add(Cut("byVLooseDeepTau2017v2p1VSjet_1>.5 &&byVLooseDeepTau2017v2p1VSjet_2>.5","VLooseTauIso"))
        ...
        self.estimation = DataEstimation(
            ...
            friend_directory=["/ceph/htautau/"+eraName+"/"+ft for ft in ["ff_friends/", "mela_friends/", "svfit_friends/"]  ] ,
    ...
    def createRDF(self):
        ...
        self.rdfFilePath="/ceph/mscham/data/fakefaktorRDFs/{}-{}.root".format(self.meta["era"],self.meta["channel"])
        if os.path.isfile(self.rdfFilePath) and os.path.exists(self.rdfFilePath):
            ...
            for i, ntupleFilename in enumerate(self.estimation.get_files()):
                ...
                for friend in self.estimation.get_friend_files() : # friend in mela, svfit, ...
                    friendFileName=friend[i]
                    if not os.path.exists(friendFileName):
                        logger.fatal("File does not exist: {}".format(friendFileName))
                        raise Exception

                    friendVarL=ROOT.RDataFrame(tree_path,str(friendFileName)).GetColumnNames()
                    ...
                    if friendName not in self.chainsD:
                        ...
                        logger.info("creating friend tree chain for "+friendName)
                    logger.info("Adding {} as {} friend.".format(friendFileName,friendName))
        ...
        logger.debug("RDF after basic cuts contains {} events.".format(self.eventCount))
        self.varL=list(self.RDF.GetColumnNames())

    def createttbarRDF(self):
        tree_path=self.channel.name+ "_nominal/ntuple"
        self.rdfttbarFilePath="/ceph/mscham/data/fakefaktorRDFs/{}-{}-ttbar.root".format(self.meta["era"],self.meta["channel"])
        if os.path.isfile(self.rdfttbarFilePath) and os.path.exists(self.rdfttbarFilePath):
        ...
        logger.debug("ttbarRDF after basic cuts contains {} events.".format(self.ttbareventCount))
        self.ttbarvarL=list(self.ttbarRDF.GetColumnNames())

    def createClosureRDF(self):
        tree_path=self.channel.name+ "_nominal/ntuple"
        FilePath="/ceph/mscham/data/fakefaktorRDFs/{}-{}-Closure.root".format(self.meta["era"],self.meta["channel"])
        ...
        logger.debug("ClosureRDF after basic cuts contains {} events.".format(self.ClosureEventCount))
        self.ClosureVarL=list(self.ClosureRDF.GetColumnNames())



class jFRegion(jetFakeEstimation):
    def __init__(self,channel,meta,RDF,signalLikeRegion,determinationRegion):
...
def plotvar(jfE,var):
    ...
    if var in jfE.RDF.GetColumnNames():
        ...
        npmat=jfE.RDF.Define("tmpplotvar",var).AsNumpy(["tmpplotvar","eventWeight"])
...
class fakefactor(object):
    def dynbins(self,arr,weights=None):
        ...
        while(0!=marker):
            curbinsum=0
            #make sure the sum of the event weights in the bin is at least 1.0
            while(curbinsum<100):
        ...
        binborders.reverse()
        return binborders
    def binbodersToBinCenters(self,arr):
    ...
    def calcyerror(self):
        ...
        yerror=np.array([self.yv[i]*np.sqrt(1/self.slh[i]+1/self.blh[i]) for i in range(len(self.yv))])
        return yerror
    def __init__(self,jfRS,filterstring,var):
        ## get the numpy arrays from the jfObject
        self.jfRS=jfRS
        self.filterstring=filterstring
        self.var = var
        self.meta=jfRS.meta
        self.blarr=jfRS.DR_bl.RDF.Filter(filterstring).AsNumpy([var,"eventWeight"])
        self.slarr=jfRS.DR_sl.RDF.Filter(filterstring).AsNumpy([var,"eventWeight"])

        ### use dynamic binning, from the bl region -> no 1/0 bins
        self.binborders = self.dynbins(self.blarr[var])
        ...
        self.fitsettings={
            ...
            "xdata":self.xv,
            "sigma":self.yerror,
            "bounds":np.array([self.pars[key][0] for key in self.pars.keys()]).T,
    ...
    def applyCorrections(self):
        c1=ClosureCorrection(self)
        print c1.var
    def calcHistograms(self):
        ### histogram both regions in the generated bins
        self.blh,temborders_= np.histogram(self.blarr["pt_2"], weights=self.blarr["eventWeight"],bins=self.binborders)
        print temborders_
        self.slh,_= np.histogram(self.slarr["pt_2"], weights=self.slarr["eventWeight"],bins=self.binborders)
        self.blh=self.blh.astype(np.float32)
        self.slh=self.slh.astype(np.float32)

        self.xv=self.binbodersToBinCenters(self.binborders)
        for i, bkgbin in enumerate(self.blh):
            if bkgbin<0.5:
                logger.fatal("Backgroundlike bin [{},{}]: {} less than .5".format(self.binborders[i],self.binborders[i+1],bkgbin))
                self.blh[i]=.1
                #raise Exception
        self.yv=self.slh/self.blh

        # x values for the fit
        self.xsc= np.arange(self.binborders[0], self.binborders[-1]+200,200./np.abs(self.binborders[-1]-self.binborders[0]))
    def getFitPars(self):
        ...
        pars["c2"]=[(np.mean(self.yv)-2*heigth,np.mean(self.yv)+2*heigth),np.mean(self.yv)]
    ...
    def bestfit(self):
        ...
        fity,fityu,fityd=[self.fitfunction(self.xsc,*par) for par in [popt, popt + self.nstd * perr, popt - self.nstd * perr]]
        print "pot pars"
        ...
        # plt.show()
        print "diag error"
        print [(key, perr[i]) for i, key in enumerate(self.pars.keys()) if perr[i]!=0]
    ...
    def roofitError(self):
        ...
        yfitvals=np.array([self.fitfunction(self.xsc,*parD_) for parD_ in parMat])
        for xi,segment in enumerate( yfitvals.T):
            fitres[xi]=np.percentile(segment,[norm.cdf(-self.nstd,0,1)*100,50,norm.cdf(self.nstd,0,1)*100])
        return fitres
    def plotFF(self):
        ...
        plt.gca().fill_between(self.xsc, self.rooFitPercV[:,2], self.rooFitPercV[:,0], alpha=.25, label=str(self.nstd)+"-sigma roofitMethod")
        plt.legend()
        plt.title(self.var+" for "+str(self.meta)+" with "+self.filterstring)
        for yvar in self.binborders:
            plt.axvline(yvar,ls=":", color="darkgrey")
        plt.tight_layout()
        plt.savefig("/home/mscham/fakeFaktors/plots/"+"-".join([self.meta[key] for key in ["era","channel","bkg"]])+"-"+self.filterstring)
        plt.show()
    # def getOldFF(self):
...
class ClosureCorrection(fakefactor):
    def __init__(self,ffObject):
        ...
        self.blarr=self.jfRS.Closure_bl.RDF.Filter(self.filterstring).AsNumpy([var,"eventWeight"])
        self.slarr=self.jfRS.Closure_sl.RDF.Filter(self.filterstring).AsNumpy([var,"eventWeight"])
        self.binborders = super.dynbins(self.blarr[var])

#%%
a=fakefactor(jfttbar,"1==1","pt_2")
...
# #plt.plot(a.binbodersToBinCenters(a.binborders),a.blh,label="new")
...
# #plt.plot(a.binbodersToBinCenters(a.binborders),a.slh,label="new")
